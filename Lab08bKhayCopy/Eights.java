...
public class Eights {
    ...
    public Eights() {
        ...
        for (int i = 0; i < arrayPlayer.size(); i++) {
            Player p = arrayPlayer.get(i);
            deck.deal(p.getHand(), 5);
        }
    ...
    public boolean isDone() {
        ...
        for (int i = 0; i < arrayPlayer.size(); i++) {
            ...
            if (p.getHand().isEmpty()) {
                System.out.println(p.getName() + " menang!");
                return true;
            }
        }
        return false;
        // return one.getHand().isEmpty() || two.getHand().isEmpty() ||
        // three.getHand().isEmpty() || four.getHand().isEmpty() ||
        // five.getHand().isEmpty();
    ...
    public Player nextPlayer(Player current) {
        // arrayPlayer bukan player 12345
        // 1 cari index player current
        // cara cari index
        // variabel utk simpen current i
        int indexCurrent = 0;
        for (int i = 0; i < arrayPlayer.size(); i++) {
            if (arrayPlayer.get(i) == current) {
                indexCurrent = i;
            }
        }
        // 2 return player index + 1
        // kalau indexnya diujung, return player ke-0
        if (indexCurrent == arrayPlayer.size() - 1) {
            return arrayPlayer.get(0);
        }
        return arrayPlayer.get(indexCurrent + 1);
    ...
    public void displayState() {
        // arrayPlayer.size().display(); salah
        // arrayPlayer tipe datanya ArrayList<Player> =
        for (int i = 0; i < arrayPlayer.size(); i++) {
            arrayPlayer.get(i).display();
        }
    ...
    public void playGame() {
        Player player = arrayPlayer.get(0); // mulai dari player pertama
