...
public class CosmicServiceImpl implements CosmicService {
    ...
    @Override
    public boolean hapusMenfess(int idHapus) {
        List<Menfess> semuaMenfess = menfessRepo.getAllMenfessesByUser(cosmic);
        Menfess target = null;
        
        for (Menfess m : semuaMenfess) {
            if (m.getId() == idHapus) {
                target = m;
                break;
            }
        ...
        if (target == null) {
            throw new MenfessNotFoundException("Menfess tidak ditemukan atau bukan milik anda, silahkan masukkan ID yang valid.");
        }

        menfessRepo.removeItem(target);
        return true;
    }

    
    @Override
    public Map<User, Integer> getLeaderboardMenfess() {
        List<Menfess> allMenfess = getAllUnhiddenMenfesses();
        Map<User, Integer> leaderboard = new HashMap<>();

        for (Menfess m : allMenfess) {
            User pengirim = m.getUser(); 
            leaderboard.put(pengirim, leaderboard.getOrDefault(pengirim, 0) + 1);
        }
